...
$state = ['GF'           =>[],                         //< global frame of variables
          ...
          'stack'        =>[],                         //< data stack
          'labels'       =>[],                         //< label stack
          'flabels'      =>[],                         //< labels mentioned but idk whrere they are
          'numLoc'       => 0,                         //< num of lines with opcodes
          'numComments'  => 0,                         //< num of comments
          'numLabels'    => count($state['labels']),   //< number of labels
...
function labelCheck($str){
  ...
  if(isset($state['labels'][$str])){
    decho(" labelCheck    \e[33mDEFINED\e[0m [$str]\n");
    return true;
  }
  decho(" labelCheck    \e[33mNOT DEFINED\e[0m [$str]\n");
  return false;
}
...
function labelCreate($str){
  ...
  if(!labelCheck($str)){
    $state['labels'][$str] = '';
    decho(" labelCreate   \e[32mGOOD\e[0m label [$str] created\n");
    return true;
  }
      ...
      switch($opcode){
        ...
        case "READ":
          ...
          if($type != "int" && $type != "string" && $type != "bool"){
            decho(" opcode READ   \e[31mFAIL\e[0m bad <type>\n");
            exit(ERR_OTHER);
          }
          if(!varSet($var, $type)){
            decho(" opcode READ   \e[31mFAIL\e[0m undefined <var>\n");
            exit(ERR_OTHER);
          }
          decho(" opcode READ   \e[32mGOOD\e[0m\n");
          break;
        case "WRITE":
          // WRITE <symb>
        ...
        case "CONCAT":
          ...
          $symb2 = $m[4];
          if(!($val1 = symbGet($symb1))){
            decho(" opcode CONCAT \e[31mFAIL\e[0m bad <symb1>\n");
            exit(ERR_OTHER);
          }
          if(!($val2 = symbGet($symb2))){
            decho(" opcode CONCAT \e[31mFAIL\e[0m bad <symb2>\n");
            exit(ERR_OTHER);
          }
          if($val1 != "string" || $val2 != "string"){
            decho(" opcode CONCAT \e[31mFAIL\e[0m bad <symb1> or/and <symb2> type\n");
            exit(ERR_OTHER);
          }
          if(!varSet($var, "string")){
            decho(" opcode CONCAT \e[31mFAIL\e[0m undefined <var>\n");
            exit(ERR_OTHER);
          }
          decho(" opcode CONCAT \e[32mGOOD\e[0m\n");
          break;
        case "STRLEN":
          // STRLEN <var> <symb>
          $var = $m[2];
          $symb = $m[3];
          if(!($val = symbGet($symb))){
            decho(" opcode STRLEN \e[31mFAIL\e[0m bad <symb>\n");
            exit(ERR_OTHER);
          }
          if($val != "string"){
            decho(" opcode STRLEN \e[31mFAIL\e[0m bad <symb> type\n");
            exit(ERR_OTHER);
          }
          if(!varSet($var, "int")){
            decho(" opcode STRLEN \e[31mFAIL\e[0m undefined <var>\n");
            exit(ERR_OTHER);
          }
          decho(" opcode STRLEN \e[32mGOOD\e[0m\n");
          break;
        case "GETCHAR":
          ...
          $symb2 = $m[4];
          if(!($val1 = symbGet($symb1))){
            decho(" opcode GETC   \e[31mFAIL\e[0m bad <symb1>\n");
            exit(ERR_OTHER);
          }
          if(!($val2 = symbGet($symb2))){
            decho(" opcode GETC   \e[31mFAIL\e[0m bad <symb2>\n");
            exit(ERR_OTHER);
          }
          if($val1 != "string" || $val2 != "int"){
            decho(" opcode GETC   \e[31mFAIL\e[0m bad <symb1> or/and <symb2> type\n");
            exit(ERR_OTHER);
          }
          if(!varSet($var, "string")){
            decho(" opcode GETC   \e[31mFAIL\e[0m undefined <var>\n");
            exit(ERR_OTHER);
          }
          decho(" opcode GETC   \e[32mGOOD\e[0m\n");
        case "SETCHAR":
          ...
          $symb2 = $m[4];
          if(!($val1 = symbGet($symb1))){
            decho(" opcode SETC   \e[31mFAIL\e[0m bad <symb1>\n");
            exit(ERR_OTHER);
          }
          if(!($val2 = symbGet($symb2))){
            decho(" opcode SETC   \e[31mFAIL\e[0m bad <symb2>\n");
            exit(ERR_OTHER);
          }
          if(!($valv = varGet($var))){
            decho(" opcode SETC   \e[31mFAIL\e[0m undefined <var>\n");
            exit(ERR_OTHER);
          }
          if($val1 != "string" || $val2 != "string" || $valv != "string"){
            decho(" opcode SETC   \e[31mFAIL\e[0m bad <symb1> or/and <symb2> and or <var> type\n");
            exit(ERR_OTHER);
          }
          if(!varSet($var, "string")){
            decho(" opcode SETC   \e[31mFAIL\e[0m undefined <var>\n");
          ...
          decho(" opcode SETC   \e[32mGOOD\e[0m\n");
          break;
        case "TYPE":
          // TYPE <var> <symb>
          $var = $m[2];
          $symb = $m[3];
          if(!($val = symbGet($smyb))){
            decho(" opcode TYPE   \e[31mFAIL\e[0m bad <symb>\n");
            exit(ERR_OTHER);
          }
          if(!varSet($var, $val)){
            decho(" opcode TYPE   \e[31mFAIL\e[0m undefined <var>\n");
            exit(ERR_OTHER);
          }
          decho(" opcode TYPE   \e[32mGOOD\e[0m\n");
          break;
        case "LABEL":
          // LABEL <label>
          $label = $m[2];
          if(!labelCreate($label)){
            decho(" opcode LABEL  \e[31mFAIL\e[0m <label> redefinition\n");
            exit(ERR_OTHER);
          }
          if(isset($state['flabels'][$label])){
            unset($state['flabels'][$label]);
            decho(" opcode LABEL  \e[33mDELETING\e[0m <label> needed before and found\n");
          }
          decho(" opcode LABEL  \e[32mGOOD\e[0m\n");
          break;
        case "JUMP":
          // JUMP <label>
          $label = $m[2];
          if(!labelCheck($label)){
            decho(" opcode JUMP   \e[33mWARN\e[0m undefined <label>\n");
            $state['flabels'][$label] = false;
          }
          decho(" opcode JUMP   \e[32mGOOD\e[0m\n");
          break;
        case "JUMPIFEQ":
          // JUMPIFEQ <label> <symb1> <symb2>
        case "JUMPIFNQE":
          // JUMPIFNEQ <label> <smyb1> <symb2>
          $label = $m[2];
          $symb1 = $m[3];
          $symb2 = $m[4];
          if(!($val1 = symbGet($symb1))){
            decho(" opcode JUMPIF \e[31mFAIL\e[0m bad <symb1>\n");
            exit(ERR_OTHER);
          }
          if(!($val2 = symbGet($symb2))){
            decho(" opcode JUMPIF \e[31mFAIL\e[0m bad <symb2>\n");
            exit(ERR_OTHER);
          }
          if($val1 != "nil" && $val2 != "nil"){
            if($val1 != $val2){
              decho(" opcode JUMPIF \e[31mFAIL\e[0m bad <symb1> or/and <symb2> type\n");
              exit(ERR_OTHER);
            }
          }
          if(!labelCheck($label)){
            decho(" opcode JUMPIF \e[33mWARN\e[0m undefined <label>\n");
            $state['flabels'][$label] = false;
          }
          decho(" opcode JUMPIF \e[32mGOOD\e[0m\n");
          break;
        case "EXIT":
          // EXIT <symb>
          $symb = $m[2];
          if(!($val = symbGet($symb))){
            decho(" opcode EXIT   \e[31mFAIL\e[0m bad <symb>\n");
            exit(ERR_OTHER);
          }
          if($val != "int"){
            decho(" opcode EXIT   \e[31mFAIL\e[0m bad <symb> type\n");
            exit(ERR_OTHER);
          }
          decho(" opcode EXIT   \e[32mGOOD\e[0m\n");
          break;
        case "DPRINT":
          // DPRINT <symb>
          $symb = $m[2];
          if(!($val = symbGet($symb))){
            decho(" opcode DPRINT \e[31mFAIL\e[0m bad <symb>\n");
            exit(ERR_OTHER);
          }
          decho(" opcode DPRINT \e[32mGOOD\e[0m\n");
          break;
        case "BREAK":
          // BREAK
          break;
...
if(count($state['flabels']) > 0){
  decho("\e[31m LABEL FAIL - LABEL NOT FOUND\e[0m\n");
  exit(ERR_OTHER);
}
